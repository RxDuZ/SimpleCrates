...
class CrateManager {
    ...
    public function __construct(){
        ...
        EntityFactory::getInstance()->register(CrateHologramEntity::class, function(World $world, CompoundTag $nbt): CrateHologramEntity
            {
                return new CrateHologramEntity(EntityDataHelper::parseLocation($nbt, $world), null, $nbt);
            }, ['CrateHologramEntity'], EntityIds::BAT
        );

        foreach($this->data->getAll() as $name => $value){
            $this->crates[$name] = new Crate($name, $value["drops"], $value["commands"], $value["floating-text"], $value["particle-color"]);
        }
    }
    ...
    public function createCrate(string $crateName): void {
        ...
        $this->crates[$crateName] = new Crate($crateName, $data["drops"], $data["commands"], $data["floating-text"], $data["particle-color"]);
    }
